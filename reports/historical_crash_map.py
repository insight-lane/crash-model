...
CRASHES_FP = RAW_FP + '/transformed_concerns_crashes/crashes/'
MAP_FP = BASE_DIR + '/data/processed/maps'
DATA_FP = BASE_DIR + '/data/processed'


def make_crash_df(city):
    with open(CRASHES_FP + city + '/transformed/crashes.json', 'r') as json_file:
        crashes_json = json.load(json_file)
    crashes_df = json_normalize(crashes_json)

    crashes_df['city'] = city

    # convert timestamp to datetime object but don't have Pandas automatically
    # adjust the timezone
    crashes_df['dateOccurred'] = crashes_df['dateOccurred'].str[:-6]
...
def make_preds_gdf(city):
    ...
    output = pd.read_csv(DATA_FP + '/seg_with_predicted_' + city + '.csv', dtype={'segment_id': str})
    ...
    if city == 'cambridge':
        MAP_FP = BASE_DIR + '/osm-cambridge/processed/maps'
        streets = gpd.read_file(MAP_FP + '/inter_and_non_int.shp')
