---
title: "Clean Mass. DOT Crash Data"
output: html_notebook
---

This notebook cleans crash data from Mass. Department of Transportation. Retreived crash data from the Mass. Department of Transportation [crash portal](http://services.massdot.state.ma.us/crashportal/DataRequest.aspx).

First, load the needed libraries and the functions needed to read in the crash data. I'm using custom functions I've made to extract multiple files at once. 
...
We can use the two functions together to read all raw csv files (which I edied slightly from original excel sheets).

```{r}
crashes <- read_multiple_csv(list_dat_dirs(getwd()))
```

Next to clean the column names.
...
Finally, an exceptionally long dplyr pipe to tidy and clean the data.
...
Then write csv: `write_csv(crashes, "Boston_data/clean/crashes_12-14.csv")`.
